...
@implementation ANHDelegateForwarder
 ...
 * the exact type of the expected delegate (i.e.: @c ANHApplicationDelegate).
 *
...
- (void)swizzleOriginalDelegate:(NSObject *)originalDelegate {
    ...
    for (NSString *selectorString in self.selectorsToSwizzle) {
        ...
        customSelector = NSSelectorFromString([kANHCustomSelectorPrefix stringByAppendingString:selectorString]);
...
- (IMP)swizzleOriginalSelector:(SEL)originalSelector withCustomSelector:(SEL)customSelector originalClass:(Class)originalClass {
    ...
    BOOL skipped = NO;
    NSString *warningMsg;
    NSString *remediationMsg = @"You need to explicitly call the Azure Notification Hub SDK from your app delegate implementation.";
    ...
    if (originalMethod) {
        ...
        if (deprecatedSelectorStr && [originalClass instancesRespondToSelector:NSSelectorFromString(deprecatedSelectorStr)]) {
            ...
            if ([self.deprecatedSelectors.allValues containsObject:originalSelectorStr]) {
                ...
                /*
                 * The original class may not implement the selector (e.g.: optional method from protocol), add the method to the original
                 * class and associate it with the custom implementation.
                 */
...
- (void)forwardInvocation:(NSInvocation *)invocation {
    @synchronized([self class]) {
        BOOL forwarded = NO;
        BOOL hasReturnedValue = ([NSStringFromSelector(invocation.selector) hasSuffix:kANHReturnedValueSelectorPart]);
        ...
        for (id<ANHCustomDelegate> delegate in self.delegates) {
