...
+ (void)startWithConnectionString:(NSString *)connectionString hubName:(NSString *)notificationHubName {
    ANHNotificationHubOptions *options = [ANHNotificationHubOptions new];
    [ANHNotificationHub startWithConnectionString:connectionString hubName:notificationHubName options:options];
}

+ (void)startWithConnectionString:(NSString *)connectionString hubName:(NSString *)notificationHubName options:(ANHNotificationHubOptions *)options {
    ANHInstallationManager *installationManager = [[ANHInstallationManager alloc] initWithConnectionString:connectionString
                         hubName:notificationHubName];

    [[ANHNotificationHub sharedInstance]
        setDebounceInstallationManager:[[ANHDebounceInstallationManager alloc] initWithInterval:2 installationManager:installationManager]];
    [[ANHNotificationHub sharedInstance] setOptions:options];

    [[ANHNotificationHub sharedInstance] registerForRemoteNotifications];
}

+ (void)startWithInstallationManagement:(id<ANHInstallationManagementDelegate>)managementDelegate {
    ANHNotificationHubOptions *options = [ANHNotificationHubOptions new];
    [ANHNotificationHub startWithInstallationManagement:managementDelegate options:options];
...
+ (void)startWithInstallationManagement:(id<ANHInstallationManagementDelegate>)managementDelegate options:( ANHNotificationHubOptions *)options {
    ANHInstallationManager *installationManager = [ANHInstallationManager new];
    [[ANHNotificationHub sharedInstance]
        setDebounceInstallationManager:[[ANHDebounceInstallationManager alloc] initWithInterval:2 installationManager:installationManager]];

    [[ANHNotificationHub sharedInstance] setManagementDelegate:managementDelegate];
    [[ANHNotificationHub sharedInstance] setOptions:options];
    [[ANHNotificationHub sharedInstance] registerForRemoteNotifications];
}
...
+ (void)didReceiveRemoteNotification:(NSDictionary *)userInfo {
    [[ANHNotificationHub sharedInstance] didReceiveRemoteNotification:userInfo fromUserNotification:NO];
}
...
+ (void)setEnabled:(BOOL)isEnabled {
    @synchronized([self sharedInstance]) {
        [[ANHNotificationHub sharedInstance] setEnabled:isEnabled];
    }
}

+ (BOOL)isEnabled {
    @synchronized([self sharedInstance]) {
        return [[ANHNotificationHub sharedInstance] isEnabled];
    }
}

- (void)setEnabled:(BOOL)isEnabled {
    [ANHLocalStorage setEnabled:isEnabled];
...
- (BOOL)isEnabled {
    return [ANHLocalStorage isEnabled];
}

+ (nullable id<ANHNotificationHubDelegate>)delegate {
    return [ANHNotificationHub sharedInstance].delegate;
}

+ (void)setDelegate:(nullable id<ANHNotificationHubDelegate>)delegate {
    [ANHNotificationHub sharedInstance].delegate = delegate;
}

#pragma mark Installations

+ (NSString *)pushChannel {
    return sharedInstance.pushChannel;
}

+ (NSString *)installationId {
    return sharedInstance.installationId;
}

- (NSString *)pushChannel {
    return self.installation.pushChannel;
}
...
- (ANHInstallation *)installation {
    ANHInstallation *installation = [ANHLocalStorage loadInstallation];

    if (!installation) {
        installation = [ANHInstallation new];
    }
...
- (void)upsertInstallation:(ANHInstallation *)installation {
    [ANHLocalStorage upsertInstallation:installation];
    ...
    if ([self isEnabled]) {
        [_debounceInstallationManager saveInstallation:installation
            ...
            completionHandler:^void(NSError *_Nullable error) {
              ...
              if (error == nil) {
                  [ANHLocalStorage upsertLastInstallation:installation];
...
+ (BOOL)addTag:(NSString *)tag {
    return [ANHNotificationHub addTags:[NSArray arrayWithObject:tag]];
}

+ (BOOL)addTags:(NSArray<NSString *> *)tags {
    return [[ANHNotificationHub sharedInstance] addTags:tags];
}

+ (void)clearTags {
    [[ANHNotificationHub sharedInstance] clearTags];
}

+ (NSArray<NSString *> *)tags {
    return sharedInstance.tags;
}

+ (BOOL)removeTag:(NSString *)tag {
    return [[ANHNotificationHub sharedInstance] removeTag:tag];
}

+ (BOOL)removeTags:(NSArray<NSString *> *)tags {
    return [[ANHNotificationHub sharedInstance] removeTags:tags];
}
...
+ (ANHInstallationTemplate *)templateForKey:(NSString *)key {
    return [sharedInstance templateForKey:key];
}

+ (NSDictionary<NSString *, ANHInstallationTemplate *> *)templates {
    return sharedInstance.templates;
}
...
- (ANHInstallationTemplate *)templateForKey:(NSString *)key {
    return [self.installation getTemplateForKey:key];
}
...
- (NSString *)userId {
    return [self installation].userId;
}
...
+ (id<ANHInstallationEnrichmentDelegate>)enrichmentDelegate {
    return [ANHNotificationHub sharedInstance].enrichmentDelegate;
}

+ (void)setEnrichmentDelegate:( id<ANHInstallationEnrichmentDelegate>)enrichmentDelegate {
    [ANHNotificationHub sharedInstance].enrichmentDelegate = enrichmentDelegate;
}

+ (id<ANHInstallationLifecycleDelegate>)lifecycleDelegate {
    return [ANHNotificationHub sharedInstance].lifecycleDelegate;
}

+ (void)setLifecycleDelegate:(id<ANHInstallationLifecycleDelegate>)lifecycleDelegate {
    [ANHNotificationHub sharedInstance].lifecycleDelegate = lifecycleDelegate;
}
