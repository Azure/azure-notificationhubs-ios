    ...
    if (requestError && error) {
      ...
      if (statusCode != 200 && statusCode != 201) {
        NSString *msg = [NSString
            stringWithFormat:@"Fail to request token. Response:%@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]];
        NSLog(@"%@", msg);
...
- (NSMutableURLRequest *)PrepareSharedSecretTokenWithUrl:(NSURL *)url {
  ...
  NSString *requestBody =
      [NSString stringWithFormat:@"wrap_scope=%@&wrap_assertion_format=SWT&wrap_assertion=%@", audienceUri, simpleWebToken];
...
- (void)setSharedSecretTokenAsync:(NSMutableURLRequest *)request completion:(void (^)(NSError *))completion {
  ...
  [[[SBURLConnection alloc] init] sendRequest:stsRequest
